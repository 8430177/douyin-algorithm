...
std::string make_ladon(uint32_t khronos, uint32_t random_num) {
    char data[32] = {0};
    std::string lc_id = "1588093228";
    std::string aid = "1128";

    // timestamp-licenseID(lc_id)-msAppID
    std::snprintf(data, sizeof data, "%u-%s-%s", khronos, lc_id.c_str(), aid.c_str());

    uint8_t keygen[8];
    memcpy(&keygen[0], &random_num, 4);
    memcpy(&keygen[4], aid.c_str(), 4);
    auto md5hex = digestpp::md5().absorb(keygen, 8).hexdigest();
